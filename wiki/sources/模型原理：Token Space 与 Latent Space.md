...
资料介绍三条新路线：BLT 根据 next-byte entropy 动态组成 Byte Patch；SuperBPE 允许跨空格形成更长的 superword；T-Free 使用 character triplet 的稀疏表示替代传统 Token Embedding。相关节省比例均来自各自实验设置，不能直接外推。
...
CoT 把推理写成可见 Token，便于监督和审计，但会增加生成成本，也不保证忠实反映内部计算。Coconut 将最后 Hidden State 作为 continuous thought 反馈给模型；Soft Thinking 使用 Token 概率分布加权 Embedding，形成连续的 abstract concept token。
