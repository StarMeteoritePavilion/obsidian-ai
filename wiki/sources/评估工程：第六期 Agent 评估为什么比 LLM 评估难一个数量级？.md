...
本期资料中的 Harness 指隔离运行、记录与评分的评估基础设施；Agent Scaffold 指运行 Agent 任务的软件编排系统，二者职责不同。共同词汇表包括 Task、Trial、Grader、Transcript、Outcome、Harness、Suite 和 Agent Scaffold。关键区分是：Transcript 保存过程，Outcome 表示环境最终状态；前者用于诊断，后者用于判定任务是否真正完成。
...
`pass@k` 衡量 k 次尝试中至少一次成功的概率，适合“一次成功就够”的产品；`pass^k` 衡量 k 次尝试全部成功的概率，适合要求每次可靠的产品。两项指标在 k 等于 1 时相同，k 增大后方向相反，因此指标选择也是产品责任的选择。若以单次成功率 p 计算 `1-(1-p)^k` 与 `p^k`，须满足各次 Trial 独立同分布；环境未重置等情况会破坏该假设。（[[raw/sources/应用工程/评估工程/第六期：Agent 评估为什么比 LLM 评估难一个数量级？#非确定性与 `pass@k` / `pass^k` 的统计|统计假设]]）
...
现存资料给出七步：从 20～50 个简单任务跑通手动闭环；明确无歧义的成功标准；覆盖高频与边缘错误；隔离环境；优先确定性评分；阅读 Transcript 定位失败起点；让达标的 Capability 进入长期 Regression。

环境隔离是其中的关键约束。共享文件、数据库或 Git 历史会让不同 Trial 相互泄漏信息，评估结果因此无法独立复现。

## 适用边界

“高一个数量级”是资料对复杂度的概括，不是跨项目通用成本倍数。SWE-bench Verified 的分数变化是资料转述，不能脱离评测时间和协议使用。

`pass@k` 与 `pass^k` 不能互相替代。前者反映搜索宽容度，后者反映连续可靠性；只报告其中一项，会掩盖另一种产品风险。

## 系列位置

资料明确标注“评估工程系列 · 第 6 期 / 共 8 期”。本期资料承接第五期对产品级评估系统的讨论，把评估对象从单次模型回复扩展到 Agent 的多轮行为和环境状态；结尾预告第七期“生产护栏”。
