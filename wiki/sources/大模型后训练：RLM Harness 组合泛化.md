...
原始材料保存在代码环境变量中，主模型只通过少量探针查看内容；子模型和工具被包装为函数，完整返回继续保存在变量中。资料展示的训练轨迹包含 78,000 字符，评测轨迹包含 618,000 字符，但第一步都是查看数据长度、开头 500 个字符和结尾 500 个字符。主 Context 主要包含代码、局部观察和分解逻辑。解法结构相同的网页检索、数据统计或跨领域任务因此可以折叠为相近轨迹，形成任务等价类。

## 实验与成本

30B 模型只在短任务上训练，评测长度扩大 8～32 倍，最长达到 2M Token。六项基准中的四项追平或超过以 GPT-5.5 为底座、未经该训练的 RLM 系统。资料展示的训练末端奖励中，直接训练组约为 0.93，RLM 组约为 0.85；长任务评测分数却分别约为 11 和 78，说明更高训练内奖励不等于更强长度泛化。

Harness 只让通用分解成为可探索路径，不保证强化学习自动找到它。一个环境中的模型曾把完整问题交给单次子调用，以短任务捷径获得奖励；研究者增加“先说明分解计划”的要求后，模型才改用可泛化方案。跨领域实验中，直接训练组只因学会答案格式获得一次有限提升，随后趋于平坦。
